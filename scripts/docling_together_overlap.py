...
def semantic_similarity_chunking(text, model, sim_threshold=0.5, max_len=1000, overlap=150):
    """Split text semantically with overlap, constrained by max length."""
    ...
    for i in range(1, len(sentences)):
        ...
        # Split if semantic similarity drops or chunk too long
        if sim < sim_threshold or current_length + sent_len > max_len:
            new_chunk = " ".join(current_chunk)
            chunks.append(new_chunk)

            # Overlap: include last N chars from previous chunk
            overlap_text = new_chunk[-overlap:] if len(new_chunk) > overlap else new_chunk
...
def extract_pdf_content(pdf_path: Path, doc_converter: DocumentConverter):
    ...
    # Extract tables and convert to linear text
    tables_data = []
    for i, table in enumerate(doc.tables, 1):
        ...
        table_as_text = f"Table {i}: " + " | ".join(linearized_rows)
        tables_data.append({
...
def main():
    ...
    for pdf_path in pdf_files:
        ...
        # Merge tables into the full text
        ...
        # Final fallback: entire text
        if not sections and len(combined_text.strip()) > 50:
            sections = [combined_text.strip()]

        # Semantic chunking with overlap
        for sec in sections:
            subchunks = semantic_similarity_chunking(sec, model, max_len=1000, overlap=150)
        ...
        logger.info(f"📄 {pdf_path.name}: {len(all_combined_chunks)} total chunks so far.")
    ...
    logger.info(f"✅ Extracted text, tables, and {len(all_combined_chunks)} combined chunks with overlap and metadata.")
