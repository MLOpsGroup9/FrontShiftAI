...
CLEANED_DATA_DIR = DATA_DIR / "cleaned"           # input from preprocessor
VALIDATED_DATA_DIR = DATA_DIR / "validated"       # output (contains chunks)
...
class ChunkGenerator:
    ...
    def extract_company_name(self, filename: str, sections: List[Dict]) -> str:
        """Extract clean company name strictly from the filename."""
        ...
        # Remove domain or industry (the first part)
        if len(parts) > 1:
            parts = parts[1:]  # everything after domain
        company_raw = ' '.join(parts).strip()

        # Clean out unwanted tokens
        doc_words = {
            'handbook', 'policy', 'manual', 'guide', 'document', 'employee',
            'team', 'member', 'updated', 'effective', 'md', 'pdf',
            'jan', 'feb', 'mar', 'apr', 'may', 'jun', 'jul', 'aug', 'sep', 'oct', 'nov', 'dec'
        }
        filtered = [w for w in company_raw.split() if w.lower() not in doc_words and not (w.isdigit() and len(w) == 4)]

        # Capitalize for readability
        company_name = ' '.join(w.capitalize() for w in filtered)
